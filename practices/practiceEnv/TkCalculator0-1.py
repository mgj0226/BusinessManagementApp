...
class Calculator(tk.Frame):
    ...
    def createWidgets(self):
        ...
        self.imgFnRoot = ImageTk.PhotoImage(file = "sqrt.png")
        self.btnFnRoot = tk.Button(self, image = self.imgFnRoot, command = self.clickFnRoot, height = 32, width = 32)
        self.lblNum.grid(row=0, column=0, columnspan=4, sticky=tk.W+tk.E)
    ...
    def setNum(self, num):
        if self.shouldClear:
            self.lblNum.configure(text = num)
            self.shouldClear = False
        else:
            self.lblNum.configure(text = self.lblNum.cget("text") + num)
